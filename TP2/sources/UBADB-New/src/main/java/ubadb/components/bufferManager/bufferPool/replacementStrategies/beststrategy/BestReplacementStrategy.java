...
public class BestReplacementStrategy implements PageReplacementStrategy
{
	...
	public BufferFrame findVictim(Collection<BufferFrame> bufferFrames) throws PageReplacementStrategyException
	{
		...
		for(BufferFrame bufferFrame : bufferFrames)
		{
			...
			if(bufferFrame.canBeReplaced() && (  futureRequestTime > longerFutureRequestTime))
			{
				victim = bufferFrame;
	...
	public int getFutureRequestTime(BufferFrame bufferFrame) {
		...
		for (PageReference pageReference : trace.getPageReferences()) {
			if (position > positionInTrace && pageReference.getPageId().equals(bufferFrame.getPage().getPageId())) {
				break;
			}
			position++;
		}
